...
└── renderer.py            # 卡片/格式化（如 AutoService 支持卡片）
```
...
class OutboundRouter:
    def route(self, conversation_id, kind, text, cs_msg_id=None):
...
```toml
[bots.autoservice]
api_endpoint = "https://your-autoservice.com/api"
credential_file = "credentials/autoservice.json"  # 含 API token / OAuth client
default_agent_template = "fast-agent"
lazy_create_enabled = true   # 新对话自动 onboard
...
supervises = ["autoservice"]

[channels."#conv-001"]
bot = "autoservice"
external_chat_id = "session_xxx"   # AutoService 的 session UUID
