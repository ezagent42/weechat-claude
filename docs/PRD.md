...
WeeChat 的 Zenoh P2P 聊天基础设施。提供 room/DM 管理、消息收发、在线状态追踪。**不知道 Claude Code 的存在**——任何 Zenoh 节点（人类、Agent、bot）对它而言都是平等的 participant。
...
wc/                                  # 根前缀
  rooms/
    {room_id}/
      messages                       # Room 消息 (pub/sub)
      presence/{nick}                # Room 成员在线状态 (liveliness)
  dm/
    {sorted_pair}/                   # DM 通道（alice_bob，字母序拼接）
      messages                       # DM 消息 (pub/sub)
  presence/
...
|命令                          |描述                                                |
|----------------------------|--------------------------------------------------|
|`/zenoh join <#room>`       |加入 room，创建 buffer，subscribe 消息，声明 liveliness token|
|`/zenoh join @<nick>`       |开启 DM，创建 buffer，subscribe DM 通道                   |
|`/zenoh leave [target]`     |离开当前或指定的 room/DM                                  |
|`/zenoh nick <n>`           |修改昵称，广播 nick 变更                                   |
|`/zenoh list`               |列出已加入的 room 和 DM                                  |
...
weechat.hook_signal_send("zenoh_message_received",
    ...
    json.dumps({"room": room, "nick": nick, "body": body}))
...
```
订阅（接收消息）:
  wc/dm/{pair}/messages         ← DM 消息（pair 包含 agent name）
  wc/rooms/{room}/messages      ← Room 消息（如果 agent 被加入 room）

发布（发送回复）:
  wc/dm/{pair}/messages         → DM 回复
  wc/rooms/{room}/messages      → Room 回复

在线状态:
  wc/presence/{agent_name}      → liveliness token
  wc/rooms/{room}/presence/{agent_name} → room 内在线状态
```
...
```python
# 创建 DM buffer → 执行 weechat-zenoh 命令
...
|`/agent join <agent> <#room>`           |让 Agent 加入 room                           |
...
See original PRD for DM, Room @mention, dynamic agent creation, agent lifecycle, and multi-user tests.
