...
└─ 后台 fork（仅检查，不下载）：
      ...
      ├─ 本地 == 远程？→ 跳过
      └─ 有更新 → 写入 update.json（update_available: true）
```

用户下次执行任何 `zchat` 命令时，如果 `update_available: true`，在输出末尾打印一行提示：

```
💡 New version available. Run `zchat upgrade` to update.
```

实际安装仅在用户显式执行 `zchat upgrade` 时发生。`zchat upgrade` 始终同时升级 zchat 和 zchat-channel-server，视为原子操作 — 任一失败则回滚（重新安装旧版本）。
