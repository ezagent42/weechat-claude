...
| `zchat/cli/app.py` | Modify | Add `zchat auth` CLI commands, update `project create` |
| `zchat/cli/irc_manager.py` | Modify | Inject auth-script into ergo config, add WeeChat SASL |
| `zchat/cli/agent_manager.py` | Modify | Pass SASL credentials to channel-server env |
...
Run: `uv run pytest tests/unit/test_project.py::test_create_project_config_with_auth tests/unit/test_project.py::test_load_project_config_defaults_auth_to_none -v`
...
Add after the `[tmux]` section in the config template string:

```python
    # Auth section (only include non-none values)
    auth_section = f'''
[auth]
...
In `load_project_config()`, add after the existing defaults:
...
In `cmd_project_create()`, after the `proxy` prompt (~line 117), add:
...
- [ ] **Step 6: Verify CLI commands register**
...
**Files:**
- Modify: `zchat/cli/agent_manager.py:124-158` (_write_workspace_config)
- Modify: `zchat/cli/app.py:63-77` (_get_agent_manager)

- [ ] **Step 1: Add project_dir param to AgentManager**

In `zchat/cli/agent_manager.py`, update `__init__` to accept `project_dir`:
...
- [ ] **Step 2: Inject SASL env vars in _write_workspace_config**

In `_write_workspace_config()`, after the existing env dict (~line 152), add:

```python
        # Add SASL credentials if OIDC auth is configured
        if self.project_dir:
            from zchat.cli.auth import get_credentials
            creds = get_credentials(self.project_dir)
            if creds:
                sasl_user, sasl_pass = creds
                server_config["env"]["IRC_SASL_USER"] = name  # scoped agent name
                server_config["env"]["IRC_SASL_PASS"] = sasl_pass
                auth_file = os.path.join(self.project_dir, "auth.json")
                server_config["env"]["AUTH_TOKEN_FILE"] = auth_file
```

- [ ] **Step 3: Update _get_agent_manager in app.py to pass project_dir**

In `zchat/cli/app.py`, update `_get_agent_manager()`:
...
```bash
git add zchat/cli/agent_manager.py zchat/cli/app.py
