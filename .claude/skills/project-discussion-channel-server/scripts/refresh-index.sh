...
# 刷新 Skill 1 (project-discussion-channel-server) 的模块索引。
# 当检测到文件移动/重命名、test-runner 失败、或新模块出现时调用。
# 重新扫描指定模块或全部模块，更新索引和 test-runner。

DRY_RUN=false
PROJECT_ROOT="/home/yaosh/projects/zchat/zchat-channel-server"
MODULE=""
ALL=false
...
Refresh Skill 1 (channel-server) module index.

Options:
  --module <name>   Refresh a specific module (agent_mcp/channel_server/feishu_bridge/plugins/meta/tests)
...
echo "=== Skill 1 (channel-server) Index Refresh ==="
...
if $ALL; then
    ...
    find "$PROJECT_ROOT/src" -name "*.py" 2>/dev/null | while read -r f; do
        echo "  Found: $(basename "$f")"
    done
    [[ -f "$PROJECT_ROOT/agent_mcp.py" ]] && echo "  Found: agent_mcp.py"
    ...
    for runner in "$(dirname "$0")"/test-*.sh; do
        ...
        # 跳过 deprecated stubs
        if grep -q "^# DEPRECATED" "$runner" 2>/dev/null; then
            echo "  Skip (deprecated): $name"
            continue
        fi
        echo "  Running: $name"
    ...
    find "$PROJECT_ROOT" -path "*/$MODULE*" -name "*.py" 2>/dev/null | while read -r f; do
        echo "  Found: $f"
    done
