...
# 验证所有脚本的 --help 和 --dry-run 模式 (channel-server skill)。
# 用于确认脚本生成正确。
...
echo "=== Self-Test (channel-server): Verifying all scripts ==="
echo ""

# test-runner 脚本：检查 --help 和 --dry-run
# 跳过 V5 废弃占位脚本（deliberately exit 1 without args）
echo "--- Test-runner scripts ---"
for script in "$SCRIPT_DIR"/test-*.sh; do
    [[ -f "$script" ]] || continue
    if grep -q "^# DEPRECATED" "$script" 2>/dev/null; then
        echo "  Skip (deprecated stub): $(basename "$script")"
        continue
    fi
    check "$script" "--help"
